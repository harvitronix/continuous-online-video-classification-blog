...
def get_data(batches, num_frames, one_hot=True):
    ...
    for batch in batches:
        ...
        with open('data/predicted-frames-' + batch + '.pkl', 'rb') as fin:
...
def main(batches, frames, batch_size, epoch):
    ...
    # {frames} frames, 2D
    # 3 outputs
    data = tf.placeholder(tf.float32, [None, frames, 3])
...
if __name__ == '__main__':
    ...
    for f in frames:
        for bs in batch_sizes:
            print("Doing %d frames, %d batch size, %d epochs" %
                  (f, bs, epochs))
            error = main(batches, f, bs, epochs)
            errors.append([f, bs, epochs, error])
